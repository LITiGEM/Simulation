function GOLIT(length, height, bound, unbound, genNum,time_pause)
    ...
    for i=1:length;
        for j=1:height;
            ...
            elseif neighbour(i,j)==1 %for 1 neighbour
                if rand<= 0.30*S_70(g,2) %generate random numbers btw 1 and 2, 50% will bind
                    M(i,j)=1;
                else M(i,j)=M(i,j);
                end
            elseif neighbour(i,j)==2;
                if rand<=0.40*S_70(g,2);
                    M(i,j)=1;
                else M(i,j)=M(i,j);
                end
            elseif  neighbour(i,j)==3;
                if rand<= 0.60*S_70(g,2);
                    M(i,j)=1;
                else M(i,j)=M(i,j);
                end
